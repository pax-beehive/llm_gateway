...
func (service *Service) BuildAccessSnapshot(
	...
	var tenant access.TenantSnapshot
	...
	if err := transaction.QueryRowContext(ctx, `
		...
		return access.Snapshot{}, ErrNotFound
	} else if err != nil {
		return access.Snapshot{}, err
	}
	...
	defer rows.Close()
	keys := make([]access.KeySnapshot, 0)
	for rows.Next() {
	...
	return access.Snapshot{Tenant: tenant, Keys: keys, CreatedAt: service.now().UTC()}, nil
}
...
func (service *Service) ActiveDigestVersionCount(ctx context.Context, actor tenantadmin.ActorEnvelope, version int16) (int64, error) {
	...
	err := service.database.QueryRowContext(ctx, `
		...
		WHERE k.digest_version = $1 AND k.status = 'active' AND t.status <> 'closed'`, version).Scan(&count)
	return count, err
}
