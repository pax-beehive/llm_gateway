...
func TestProjectionDeduplicatesRejectsStaleDetectsGapAndAuthenticatesLocally(t *testing.T) {
	...
	event := func(id string, revision int64, status access.APIKeyStatus) accessprojection.ControlEvent {
		payload, marshalErr := json.Marshal(map[string]any{
			"tenant_id": tenantID, "tenant_status": access.TenantActive, "tenant_revision": 1,
	...
	applied, err := store.Apply(ctx, event("event-1-"+tenantID, 1, access.APIKeyActive))
	if err != nil || applied.Disposition != accessprojection.DispositionApplied || applied.Lag != time.Second {
		t.Fatalf("apply = %#v err=%v", applied, err)
	}
	duplicate, err := store.Apply(ctx, event("event-1-"+tenantID, 1, access.APIKeyActive))
	...
	if _, err := store.Apply(ctx, tenantEvent("tenant-event-2-"+tenantID, 2, access.TenantSuspended)); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.Apply(ctx, tenantEvent("tenant-event-3-"+tenantID, 3, access.TenantActive)); err != nil {
		t.Fatal(err)
	}
...
func TestSnapshotAtomicallyRepairsProjectionAndGatesPepperRetirement(t *testing.T) {
	...
	digest := hmac.New(sha256.New, pepper)
	_, _ = digest.Write([]byte(rawKey))
	if err := store.ReplaceSnapshot(ctx, accessprojection.Snapshot{
		...
		Keys: []accessprojection.KeySnapshot{{
			ID: "gak_" + tenantID, Prefix: "gw_snapshot", SecretDigest: digest.Sum(nil), DigestVersion: 2,
